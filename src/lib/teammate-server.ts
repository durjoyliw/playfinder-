...
import type { TeammateUser } from "@/lib/teammate";

// Split out from teammate.ts (server-only: pulls in the Prisma client, which
// itself pulls in web-push for the push-notification extension). Kept
// separate so client components can import the pure formatTeammateSportLabels
// helper / TeammateUser type from teammate.ts without dragging prisma/web-push
