...
/**
 * Computes trending topics per sport using a TF-IDF-style relative "lift"
 * score, so a word only counts as trending for a sport if it's unusually
 * common there right now relative to its normal baseline rate everywhere
 * (avoids ubiquitous words like a city name always winning). Shared by the
 * /api/discover/trending route and the nightly notifications cron job.
 */
